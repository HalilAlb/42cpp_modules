...
void testAbstractClass()
{
	...
	std::cout << "Note: Animal class is now abstract (pure virtual makeSound())" << std::endl;
	std::cout << "Trying to instantiate Animal directly will cause compilation error!" << std::endl;
	std::cout << "\n// Animal animal;              // ✗ ERROR: Cannot instantiate abstract class" << std::endl;
	std::cout << "// Animal* ptr = new Animal(); // ✗ ERROR: Cannot instantiate abstract class" << std::endl;
	std::cout << "\nBut we CAN use Animal pointers to point to Dog/Cat objects:" << std::endl;
	std::cout << "Animal* ptr = new Dog();      // ✓ OK: Polymorphism works!" << std::endl;
}
